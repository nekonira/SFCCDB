...
foreach ($l in $lines) {
    if ($l.Contains("p01") -and $l.Contains("p50") -and $l.Contains("p100")) {
        if ($l.Length -gt $maxHits) {
            $maxHits = $l.Length
            $bestLine = $l
        }
    }
...
if ($bestLine.Length -gt 1000) {
    Write-Host "Found HUGE full database line! Length: $($bestLine.Length)"
    ...
    if ($start -gt 0) {
        ...
        if ($end -gt 0) {
            ...
            Write-Host "HUGE full database restored successfully!"
        }
    }
} else {
    Write-Host "Searching alternative logs..."
}
