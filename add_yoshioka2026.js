...
console.log('--- Adding / Updating Keita Yoshioka 2026 (p154) ---');

const yoshioka2026Obj = `{
    id: 'p154',
    name: '喜岡佳太(2026)',
    readingName: 'よしおかけいた',
    category: 'DF',
...
// Check if p154 already exists in mockData.js
if (code.includes("id: 'p154'")) {
  code = code.replace(/\{\s*id:\s*['"]p154['"][\s\S]*?avatarUrl:\s*['"][^'"]*['"]\s*\}/, yoshioka2026Obj.trim());
} else {
  const insertPos = code.lastIndexOf('];');
  if (insertPos > 0) {
    code = code.slice(0, insertPos) + ',\n' + yoshioka2026Obj.trim() + '\n' + code.slice(insertPos);
  }
}
...
try {
  ...
  console.log('SUCCESS! Updated Keita Yoshioka 2026 (p154) in add_yoshioka2026.js');
} catch (err) {
