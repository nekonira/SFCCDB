...
console.log('=== REBUILDING ALL PLAYER AVATAR RESOLVERS IN APP.JS & APP.JSX ===');
...
const resolverCode = `const PLAYER_IMAGE_MAP = ${mapJson};
...
function replaceResolverInFile(filePath) {
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    ...
    code = before + resolverCode.trim() + after;
    fs.writeFileSync(filePath, code, 'utf-8');
    console.log(`Updated getPlayerAvatarUrl in ${path.basename(filePath)}`);
  } else {
    console.error(`Could not find markers in ${path.basename(filePath)}`);
  }
}

replaceResolverInFile(path.join(__dirname, 'src', 'app.js'));
replaceResolverInFile(path.join(__dirname, 'src', 'app.jsx'));
