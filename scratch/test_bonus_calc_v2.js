...
console.log('--- Test Matching on Key Players ---');

const testPlayers = [
  players.find(p => p.name === 'ペレ') || players[0],
  players.find(p => p.name === 'クリスティアーノ・ロナウド') || players[1],
  players.find(p => p.name.includes('ベリンガム')) || players[2],
  players.find(p => p.name.includes('遠藤')) || players[3],
  players.find(p => p.name.includes('上田綺世')) || players[4]
];

testPlayers.forEach(p => {
  console.log(`\nPlayer: ${p.name} (${p.mainPosition} / ${p.playStyle} / ${p.nationality})`);
  cards.slice(0, 10).forEach(c => {
    const mult = calculateCardBonusMult(p, c);
    if (mult > 1.0) {
      console.log(`  -> Card: ${c.name} [${c.playstyleBonus?.style}] => Bonus Mult: ${mult}x (+${Math.round((mult-1)*100)}%)`);
    }
  });
});
