...
function checkBonusMatch(player, rawStyle) {
  ...
  for (const part of parts) {
    ...
    // 4. PlayStyle Specific Match ONLY (No position/category fallbacks!)
    if (pStyle) {
      if (pStyle === s || pStyle.includes(s) || (s.length >= 3 && pStyle.length >= 3 && s.includes(pStyle))) return true;
...
players.forEach(p => {
  cards.forEach(c => {
    ...
    if (mult > 1.0) totalMatches++;
  });
});

console.log(`Evaluated ${totalEvaluations} player x card combinations.`);
console.log(`Total Bonus Matches found: ${totalMatches} (Avg ${(totalMatches / players.length).toFixed(1)} matching cards per player out of ${cards.length} cards)`);
