...
console.log('--- Verifying p390 integration in mockData.js & app.js ---');
...
// 1. Load Image JS
const imgJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'data', 'raulJimenez2026Image.js'), 'utf-8');
vm.runInContext(imgJs, sandbox);
console.log('Image JS loaded, RAUL_JIMENEZ_2026_IMAGE exists:', Boolean(sandbox.window.RAUL_JIMENEZ_2026_IMAGE));
...
const player = players.find(p => p.id === 'p390');
console.log('p390 Player details:');
...
if (mapMatch) {
  ...
  console.log('PLAYER_IMAGE_MAP p390 value:', mapObj['p390']);
}
