...
console.log('--- Verifying p391 (Cody Gakpo) integration ---');
...
// 1. Load Image JS
const imgJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'data', 'codyGakpo2026Image.js'), 'utf-8');
vm.runInContext(imgJs, sandbox);
console.log('Image JS loaded, CODY_GAKPO_2026_IMAGE exists:', Boolean(sandbox.window.CODY_GAKPO_2026_IMAGE));
...
const player = players.find(p => p.id === 'p391');
console.log('p391 Player details:');
...
if (mapMatch) {
  ...
  console.log('PLAYER_IMAGE_MAP p391 value:', mapObj['p391']);
}
