...
console.log('--- Verifying p392 (Leandro Trossard) integration ---');
...
// 1. Load Image JS
const imgJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'data', 'leandroTrossard2026Image.js'), 'utf-8');
vm.runInContext(imgJs, sandbox);
console.log('Image JS loaded, LEANDRO_TROSSARD_2026_IMAGE exists:', Boolean(sandbox.window.LEANDRO_TROSSARD_2026_IMAGE));
...
const player = players.find(p => p.id === 'p392');
console.log('p392 Player details:');
...
if (mapMatch) {
  ...
  console.log('PLAYER_IMAGE_MAP p392 value:', mapObj['p392']);
}
