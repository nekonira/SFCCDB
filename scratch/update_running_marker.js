...
console.log('=== UPDATING ABILITY DESCRIPTION FOR ランニングマーカー ===');

// 1. Update src/data/mockData.js
const mockPath = path.join(__dirname, '..', 'src', 'data', 'mockData.js');
let mockCode = fs.readFileSync(mockPath, 'utf-8');

const targetStr = "{ name: 'ランニングマーカー', rank: '銅', description: '発動条件：好調　/　マーク・走力UP' }";
const newStr = "{ name: 'ランニングマーカー', rank: '銅', description: '発動条件：絶好調　/　マーク・走力UP' }";

if (mockCode.includes(targetStr)) {
  mockCode = mockCode.replace(targetStr, newStr);
  fs.writeFileSync(mockPath, mockCode, 'utf-8');
  console.log('Updated ランニングマーカー description in mockData.js');
}
...
if (fs.existsSync(scriptPath)) {
  ...
  if (scriptCode.includes(targetStr)) {
    ...
    console.log('Updated ランニングマーカー description in add_daizenMaeda2026.js');
  }
}
